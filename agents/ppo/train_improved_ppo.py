...
def train_improved_ppo_agent(
    ...
    model_name: str = "ppo_improved"
) -> PPO:
    """
    Train improved PPO agent with curriculum learning and opponent modeling
    """
    
    print("🧠 Training Improved PPO Agent with Curriculum Learning")
    ...
    if log_dir is None:
        log_dir = repo_root / "results" / "ppo_improved"
    ...
    evaluation_results = evaluate_improved_model(
...
def evaluate_improved_model(
    ...
    """Evaluate improved model against all opponent strategies"""
    ...
    if log_dir:
        ...
        results_df.to_csv(f"{log_dir}/improved_evaluation_results.csv")
        
        # Plot results
        plot_improved_results(results, log_dir)
...
def plot_improved_results(results: Dict, log_dir: str):
    """Plot improved evaluation results"""
    ...
    ax1.set_title('Improved PPO: Performance vs Different Opponents')
    ...
    ax2.set_title('Improved PPO: Cooperation Rate vs Different Opponents')
    ...
    ax3.set_title('Improved PPO: Strategic Adaptation')
    ...
    baseline_rewards = [299.0, 302.0, 1.0, 151.5, 299.0, 5.0, 299.4]  # Original PPO results
    improvement = [r - b for r, b in zip(rewards, baseline_rewards)]
    colors = ['green' if i > 0 else 'red' for i in improvement]
    
    bars4 = ax4.bar(strategies, improvement, alpha=0.8, color=colors)
    ax4.set_ylabel('Improvement vs Original PPO')
    ax4.set_title('Performance Improvement over Original PPO')
    ...
    for bar, imp in zip(bars4, improvement):
        height = bar.get_height()
        ax4.text(bar.get_x() + bar.get_width()/2., height + (1 if height > 0 else -5),
                f'{imp:+.1f}', ha='center', va='bottom' if height > 0 else 'top')
    
    plt.tight_layout()
    plt.savefig(f"{log_dir}/improved_ppo_evaluation.png", dpi=150, bbox_inches='tight')
    plt.close()


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train improved PPO agent for IPD")
    ...
    print("=== Improved PPO Training for Iterated Prisoner's Dilemma ===")
    
    # Train improved model
    model = train_improved_ppo_agent(
        total_timesteps=args.timesteps,
        learning_rate=args.learning_rate,
        num_rounds=args.num_rounds,
        seed=args.seed,
        model_name="ppo_improved"
    )
    
    print("\n✅ Improved PPO Training completed!")
    print(f"🎯 Model trained for {args.timesteps + 100000:,} timesteps")
    print(f"📊 Results saved in results/ppo_improved/")
    print(f"💾 Model saved in models/ppo_improved") 